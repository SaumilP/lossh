...
def get_restriction_line(port=52222,
                         public_key=None,
                         denied_msg="'lossh account only allows port forwarding.'",
                         denies="no-X11-forwarding,no-agent-forwarding,no-pty"):
    if not public_key:
        raise ValueError("public_key should be set? or password method")
...
def read_restrictions(auth_key_file):
    restrictions = ""
...
def create_osx_user(user=USER):
    user_dir = get_user_dir(user)
    uid = get_random_uid()
    ensure_rbash_osx()
    os.system("sudo dscl . -create /groups/lossh && sudo dscl . -append /groups/lossh gid 622")
    cmds = ['sudo dscl . -create /Users/{0} UniqueID "{1}"',
            'sudo dscl . -create /Users/{0}',
            'sudo dscl . -create /Users/{0} RealName "Lo SSH"',
...
def create_user(user):
    os_dependent_functions = {"Darwin": create_osx_user, "Linux": create_linux_user}
...
def remove_user(user):
    """ Used to remove a lossh user """
    if user in os.path.expanduser("~"):
        raise Exception("Cannot delete self")
    if not user.startswith("lossh"):
        _input = input("Are you sure you want to delete user '{}' [y/N]: ".format(user))
        if _input.lower() != "y":
            print("Aborting.")
            return
...
@click.option("--background", "-b", default=52222)
@click.option("--local_part", "-l", default=52222)
        ...
        for line in f:
            ...
            for port in re.findall(r"127.0.0.1:(\d+)", line):
                if "ssh-rsa" not in line:
                    print(line)
                    msg = "Please post an issue with above line"
                    raise Exception(msg)
...
def ls():
    ...
    if not on_apple:
        print("To read lossh folder you need root access on your machine.")
